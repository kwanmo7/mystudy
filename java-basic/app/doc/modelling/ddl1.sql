...
-- 주소
DROP TABLE IF EXISTS lms_adress RESTRICT;
...
CREATE TABLE lms_application (
	...
	register_dt    DATE    NOT NULL DEFAULT now() COMMENT '신청일', -- 신청일
...
CREATE TABLE lms_managers (
	...
	department_no INTEGER     NULL     COMMENT '부서번호' -- 부서번호
)
COMMENT '매니저';
...
-- 주소
ALTER TABLE lms_adress
	ADD CONSTRAINT PK_lms_adress -- 주소 기본키
	PRIMARY KEY (
	adress_no -- 주소번호
	);

-- 주소 인덱스
CREATE INDEX IX_lms_adress
	ON lms_adress( -- 주소
		postno ASC -- 우편번호
	);
...
-- 학생
ALTER TABLE lms_students
	ADD CONSTRAINT FK_lms_adress_TO_lms_students -- 주소 -> 학생
	FOREIGN KEY (
	adress_no -- 주소번호
	)
	REFERENCES lms_adress ( -- 주소
	adress_no -- 주소번호
	);
