...
public class DispatcherServlet extends HttpServlet {
  ...
  public void init() throws ServletException {
    try {
      ...
      for (String bean : beanNames) {
        beans.add(applicationContext.getBean(bean));
      }
  ...
  private Object[] prepareRequestHandlerArguments(Method handler, HttpServletRequest request,
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (parameter.getType() == HttpServletRequest.class
        ...
        if (cookieValue != null) {
          String value = getCookieValue(cookieValue.value(), request);
          if (value != null) {
            args[i] = valueOf(value, parameter.getType());
          }
          continue;
        }
        ...
        if (requestParam != null) {
          ...
          if (parameter.getType() == Part[].class) {
            ...
            args[i] = valueOf(paramValue, parameter.getType());
          }
          continue;
        }
        // 파라미터 타입이 도메인 클래스일 경우 해당 클래스의 객체를 준비하여
        // 그 객체에 요청 파라미터 값들을 담은 후 저장
        args[i] = createValueObject(parameter.getType(), request);
      }
    }
    return args;
  }

  // 문자열을 주어진 타입으로 변환하여 리턴
  ...
  // 해당 클래스의 객체를 생성하고 요청 파라미터 값을 담아서 리턴
  private Object createValueObject(Class<?> type, HttpServletRequest request) throws Exception {
    ...
    // 도메인 객체를 생성한 후
    // 도메인 객체의 프로퍼티 이름과 일치하는 요청 파라미터 값을 담아줌
    ...
    for (Method setter : methods) {
      ...
      // 세터 메서드의 이름에서 프로퍼티 이름을 추출
      // ex) setFirstName ==> firstName
      String propName =
          Character.toLowerCase(setter.getName().charAt(3)) + setter.getName().substring(4);

      // 프로퍼티 이름으로넘어온 요청 파라미터 값을 꺼낸다
      String requestParamValue = request.getParameter(propName);

      // 도메인 객체의 프로퍼티 이름과 일치하는 요청 파라미터 값이 있다면 객체에 저장
      if (requestParamValue != null) {
        setter.invoke(obj, valueOf(requestParamValue, setter.getParameters()[0].getType()));
      }
    }
    return obj;
  }
